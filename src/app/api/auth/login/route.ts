...
import { consumeAuthChallenge, createSession, CURRENT_TERMS_VERSION, getOrCreateUser } from '@/lib/auth';
import { requestFingerprint } from '@/lib/risk';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!nonce || signature.length !== 64) return NextResponse.json({ error: 'Invalid authentication request' }, { status: 400 });
    ...
    const valid = nacl.sign.detached.verify(new TextEncoder().encode(message), signature, new PublicKey(walletAddress).toBytes());
    if (!valid) return NextResponse.json({ error: 'Invalid signature' }, { status: 401 });

    const user = await getOrCreateUser({
      walletAddress,
      refCode: typeof body.refCode === 'string' ? body.refCode.trim() : null,
      inviteCode: typeof body.inviteCode === 'string' ? body.inviteCode.trim() : null,

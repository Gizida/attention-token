...
export async function POST(request: NextRequest) {
  try {
    ...
    const identity = requestFingerprint(request) || walletAddress;
    const rate = await enforceRateLimit({ identity, bucket: 'auth_challenge', limit: 10, windowSeconds: 600 });
    ...
    return NextResponse.json({ nonce: challenge.nonce, message: challenge.message, expiresAt: challenge.expiresAt.toISOString() });
  } catch {
